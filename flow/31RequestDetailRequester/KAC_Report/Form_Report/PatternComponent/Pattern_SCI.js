...
exports.DataSetTH = async (dataReport, doc, currentY,specialAddPage) => {
  try {
    console.log("DataSet");
    ...
    for (startIndex = 0; startIndex < dataReport.length; startIndex++) {
      if (countRound == specialAddPage) //common 3
       {
      ...
      doc.setFont("THSarabun", "bold");
      doc.text("วันที่ทำรายงาน", 120, currentY + 8);
      doc.setFont("THSarabun", "normal");
      doc.text(
      ...
      doc.setFont("THSarabun", "bold");
      doc.text("ค่าควบคุมน้ำยา", posSub, currentY + 8);
      currentY = currentY + fontSize / 2.5;
