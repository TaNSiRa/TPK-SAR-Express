...
exports.CoatingPre = async (dataReport, reportNumStart, doc, currentY) => {
  try {
    console.log("Coat_PreNBCT");
    var i = 0;
    for (var i = 0; i < dataReport.length; i++) {
      if (dataReport[i].ReportOrder == reportNumStart) {
        break;
      }
    }
    var indexStart = i;
    doc.autoTable({
      ...
      head: [
        [
          {
            content: "Material : " + dataReport[indexStart].ProcessReportName,
            styles: {
              textColor: 0,
              halign: "left",
              valign: "middle",
    ...
    var layer = ["Stearate Film", "Metal Soap Film", "Phosphate Film"];
    // max 3 data
    var y = 0;
    ...
    doc.autoTable({
      ...
      head: [
        [
          ...
          { colSpan: 4, content: "coating weight 3 layer (g/m²)" },
        ],
        [
          {
            content: "No.1",
