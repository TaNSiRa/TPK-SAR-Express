...
exports.HeaderSetTH = async (dataReport, doc) => {
  try {
    ...
    doc.text("รายงานการตรวจสอบกระบวนการพรีทรีทเมนต์", 61, currentY + 4);

    currentY = currentY + fontSize / 2.5;
    doc.setFontSize(15);
    doc.text("วันที่เก็บตัวอย่าง", 79, currentY + 4);
    doc.text(dtConv.toDateOnly(dataReport[0].SamplingDate), 114, currentY + 4);

    currentY = currentY + fontSize / 2.5;
    doc.text("วันที่ทำรีพอร์ต", 79, currentY + 3);
    doc.text(
...
exports.SignSetTH = async (dataReport, doc, currentY) => {
  try {
    console.log("SignSet");
    //spans();
    var signCount = 0;
