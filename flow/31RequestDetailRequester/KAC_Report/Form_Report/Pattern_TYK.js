...
exports.CreatePDF = async (dataReport) => {
  try {
    ...
    //Data Set
    buffDoc = await DataSetTYK(dataReport, doc, currentY);
    doc = buffDoc[0];
    currentY = buffDoc[1];
...
async function DataSetTYK(dataReport, doc, currentY) {
  try {
    console.log("DataSetTYK");
    doc.autoTable({
      startY: currentY - 4,
      head: [
        [
    ...
    var MachineName = ["Case", "Plate\nreinforce"];
    var MachineSpans = [5, 14];
