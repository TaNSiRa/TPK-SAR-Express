...
exports.CreatePDF = async (dataReport) => {
  try {
    ...
    //
    let buff1 = dataBuffSet[j].slice(3, 4);
    var buff2 = dataBuffSet[j].slice(2, 1);
    for (var j = 0; j < dataBuffSet.length; j++) {
      ...
      dataBuffSet[j][4].ProcessReportName = "Degreasing ( FC-E2032T )";
    ...
    //Pic Set
    buffDoc = await PicSetHONDAP(dataBuffSet, currentRound, doc, currentY);
    doc = buffDoc[0];
    currentY = buffDoc[1];
...
async function PicSetHONDAP(dataReport, CurrentRound, doc, currentY) {
  try {
    console.log("PicSetHONDAPPP");
    ...
    for (j = 0; j < CurrentRound; j++) {
      ...
      for (i = picStartIndex + 3; i < dataReport[j].length; i) {
        if (j == 0 && dataReport[j][i].ReportOrder == 107) {
          dataInTable.push([
            {
